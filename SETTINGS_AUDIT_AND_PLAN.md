...
| Site Settings | Notifications / Camera / Microphone subpages | Default behavior works, but there is no per-site exception list, revoke flow, or current-site editor. |
...
| Protection | `protection.blockFingerprinting` | Persisted, but no mitigation logic. |
...
| Accessibility | `accessibility.highContrast` | Persisted, but not applied to app or page rendering. |
| System | `system.backgroundApps` | Persisted, but no background-process policy. |
| System | `Proxy settings` | No proxy settings page or network stack integration. |
...
3. Add explainability or diagnostics for blocked Protection requests if the product wants visible feedback.

### Phase 3: Wire Accessibility And System

1. Implement `accessibility.highContrast` as both app-theme adjustments and page-level CSS/forced-color strategy.
2. Implement `accessibility.caretBrowsing` if Qt WebEngine supports it, otherwise remove the setting until it is supportable.
3. Implement `system.backgroundApps` or remove the UI until there is a background task model.
4. Add a real proxy settings page and connect it to Qt network / WebEngine proxy configuration.
...
4. Turn the Ghost dashboard into a real information surface with user-facing news and weather updates, closer to the Brave-style start page direction.
